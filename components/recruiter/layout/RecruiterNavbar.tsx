...
export default function RecruiterNavbar({
  ...
  const getPageTitle = () => {
    ...
    if (pathname === "/recruiter/dashboard") return "Recruiter Dashboard";
    if (pathname === "/recruiter/jobs/create") return "Post New Job";
    if (pathname.startsWith("/recruiter/jobs")) return "My Job Listings";
    if (pathname.startsWith("/recruiter/applications")) return "Applicant Applications";
  ...
  return (
    <header className="h-16 border-b border-zinc-200 dark:border-zinc-800/80 bg-white/70 dark:bg-zinc-950/70 backdrop-blur-md sticky top-0 z-20 flex items-center justify-between px-4 md:px-6 transition-colors">
      ...
      {/* Right: Actions & Recruiter Profile Dropdown */}
          ...
          >
            {user?.profileImage ? (
              <img
                ...
                alt={user.name || "Recruiter Avatar"}
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-2 w-64 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
              ...
              <div className="py-1 px-1.5 space-y-0.5">
                ...
                >
                  <PlusCircle className="w-4 h-4 text-teal-600 dark:text-purple-400" />
