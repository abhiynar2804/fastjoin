...
const links = [
  ...
  { name: "Saved Jobs", href: "/student/saved-jobs", icon: Bookmark },
  { name: "Applications", href: "/student/applications", icon: FileCheck },
...
export default function Sidebar({
  ...
  const sidebarContent = (
    <div className="h-full flex flex-col justify-between py-5 px-3">
      ...
      <div>
        <div className="flex items-center justify-between px-3 mb-6 h-10">
          <Link href="/student/dashboard" className="flex items-center gap-3 group">
            ...
            {!isCollapsed && (
              <span className="font-bold text-xl tracking-tight text-zinc-900 dark:text-zinc-100 transition-opacity duration-200">
                FastJoin
              </span>
            )}
          </Link>
        ...
        <nav className="space-y-1.5 px-1">
          {links.map((link) => {
            ...
            const isActive =
              pathname === link.href ||
              (link.href !== "/student/dashboard" && pathname?.startsWith(link.href));
      ...
      {/* Footer Info Badge */}
      {!isCollapsed && (
        <div className="p-3 mx-1 rounded-xl bg-zinc-100 dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800/80 text-xs text-zinc-500 dark:text-zinc-400">
          <p className="font-medium text-zinc-800 dark:text-zinc-200">Student Portal</p>
          <p className="text-[11px] text-zinc-500 dark:text-zinc-400 mt-0.5">Placement Season 2026</p>
        </div>
      )}
    </div>
  );
