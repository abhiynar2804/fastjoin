    ...
    <div className="h-full flex flex-col justify-between py-5 px-3">
          ...
          <Link href="/admin/dashboard" className="flex items-center gap-3 group">
            ...
            {!isCollapsed && (
              <div>
                ...
                <span className="text-[10px] font-semibold uppercase tracking-wider text-rose-600 dark:text-purple-400 block leading-none">
                  Admin Panel
                </span>
              </div>
            )}
      ...
      {!isCollapsed && (
        <div className="p-3 mx-1 rounded-xl bg-zinc-100 dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800/80 text-xs text-zinc-500 dark:text-zinc-400 flex items-center gap-2.5">
          <ShieldCheck className="w-4 h-4 text-teal-600 dark:text-purple-400 shrink-0" />
          <div>
            <p className="font-semibold text-zinc-800 dark:text-zinc-200">System Admin</p>
            <p className="text-[11px] text-zinc-500 dark:text-zinc-400">Full Platform Control</p>
          </div>
        </div>
      )}
    </div>
